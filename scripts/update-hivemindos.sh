...
start_dashboard() {
  ...
  mkdir -p "$ROOT/.next"
  refresh_tool_paths
  nohup "$ROOT/scripts/run-with-memory-limit.sh" --limit-mb 5000 -- pnpm exec next dev --webpack -p "$PORT" > "$ROOT/.next/hivemindos.log" 2>&1 &
  sleep 2
}
...
detect_hivemind_link_enabled() {
  ...
  if [[ "$(uname -s)" == "Darwin" ]] && launchctl print "gui/$(id -u)/com.hivemindos.linkd" >/dev/null 2>&1; then
    printf "true\n"
    return
  fi
