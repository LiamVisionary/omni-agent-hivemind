...
- Summary: Rewrite root-relative HTML asset paths for peer-hosted apps on both direct peer responses and root fallback responses, remember the app-proxy target for root-relative API calls, and tunnel hosted-app WebSocket upgrades so ComfyUI Mobile can open through Z-Image Studio instead of freezing on its loading screen.
