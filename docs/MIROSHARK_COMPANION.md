...
MiroShark support is optional. OpenClaw owns real fleet coordination; MiroShark
can run beside it as a scenario rehearsal and swarm simulation engine.

## Integration Model

- OpenClaw checks `/health` on the configured or detected backend.
- OpenClaw detects common local MiroShark installs, including `MIROSHARK_HOME`,
...
- If OpenClaw has `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, or `LLM_API_KEY`, it
  can write MiroShark's local `.env` automatically with private file mode.
...
Keep operational swarm state inside OpenClaw:
